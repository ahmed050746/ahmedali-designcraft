...
export function ProjectGrid({ limit }: { limit?: number }) {
  return <div className="mt-10 grid gap-x-8 gap-y-14 md:grid-cols-12">{projects.slice(0, limit).map((project, index) => <article key={project.slug} className={`group md:col-span-${index % 3 === 0 ? "7" : "5"}`} style={{ gridColumn: undefined }}><Link to="/work/$slug" params={{ slug: project.slug }}><ProjectVisual project={project}/><div className="mt-5 grid grid-cols-[minmax(0,1fr)_auto] items-start gap-3"><div className="min-w-0"><h3 className="font-serif text-2xl">{project.name}</h3><p className="mt-1 font-mono text-[10px] uppercase tracking-[0.14em] text-ink-soft">{project.category}</p></div><ArrowUpRight className="mt-1 size-5 shrink-0 text-amber transition-transform group-hover:translate-x-1 group-hover:-translate-y-1"/></div><p className="mt-3 max-w-[58ch] text-sm leading-6 text-ink-soft">{project.description}</p><p className="mt-3 font-mono text-[10px] uppercase tracking-[0.12em]">Role — {project.role}</p></Link></article>)}</div>;
}
