...
function HeroVisual() { return <div className="rounded-lg border border-paper/20 bg-paper/10 p-1.5"><div className="overflow-hidden rounded-md bg-paper text-ink"><div className="flex items-center gap-1.5 border-b border-line px-3 py-2"><span className="size-2 rounded-full bg-line"/><span className="size-2 rounded-full bg-line"/><span className="size-2 rounded-full bg-amber/70"/><span className="ml-2 font-mono text-[9px] text-ink-soft">product / overview</span></div><div className="grid grid-cols-4 gap-2 p-3"><div className="col-span-3 rounded bg-paper-2 p-3"><div className="h-2 w-20 bg-line"/><div className="mt-4 flex h-20 items-end gap-1">{[30,55,42,76,60,90,68,82].map((h,i)=><span key={i} className="flex-1 bg-amber/55" style={{height:`${h}%`}}/>)}</div></div><div className="rounded bg-amber p-2"><span className="block h-2 w-3/4 bg-paper/50"/><span className="mt-3 block h-8 w-8 rounded-full border-4 border-paper/55"/></div>{[1,2,3,4].map(x=><div key={x} className="h-11 rounded border border-line bg-paper"/>)}</div></div></div> }
