...
export function SiteHeader() {
  ...
  return <header className="fixed inset-x-0 top-0 z-50 border-b border-line bg-paper/95 backdrop-blur-sm">
    <div className={`${pageWrap} grid h-16 grid-cols-[minmax(0,1fr)_auto] items-center gap-4`}>
      <Link to="/" className="group flex min-w-0 items-center gap-3" aria-label="Ahmed Ali — Home"><span className="grid size-8 shrink-0 place-items-center border border-ink font-mono text-[10px] font-medium transition-colors group-hover:bg-ink group-hover:text-paper">AA</span><span className="min-w-0"><span className="block truncate font-sans text-[14px] font-medium leading-none">Ahmed Ali</span><span className="mt-1 hidden font-mono text-[9px] uppercase text-ink-soft sm:block">UI/UX Engineer</span></span></Link>
      <nav className="hidden h-full items-center gap-7 md:flex" aria-label="Primary navigation">{nav.map(([label, to]) => <Link key={to} to={to} className="relative flex h-full items-center font-mono text-[10px] uppercase text-ink-soft transition-colors after:absolute after:inset-x-0 after:bottom-0 after:h-px after:origin-left after:scale-x-0 after:bg-amber after:transition-transform hover:text-ink hover:after:scale-x-100" activeProps={{ className: "text-ink after:scale-x-100" }}>{label}</Link>)}<Button asChild size="sm"><Link to="/contact">Let&apos;s Talk <ArrowUpRight /></Link></Button></nav>
    ...
    {open && <nav className={`${pageWrap} border-t border-line bg-paper py-3 md:hidden`} aria-label="Mobile navigation">{nav.map(([label, to], index) => <Link key={to} to={to} className="grid grid-cols-[2rem_1fr_auto] items-center border-b border-line py-3.5 font-mono text-xs uppercase last:border-0" activeProps={{ className: "text-amber" }}><span className="text-[9px] text-ink-soft">0{index + 1}</span><span>{label}</span><ArrowUpRight className="size-3.5" /></Link>)}</nav>}
  </header>;
}
...
export function ProjectGrid({ limit }: { limit?: number }) {
  return <div className="mt-10 grid gap-x-8 gap-y-16 md:grid-cols-12">{projects.slice(0, limit).map((project, index) => <article key={project.slug} className={`group ${index % 3 === 0 ? "md:col-span-7" : "md:col-span-5"}`}><Link to="/work/$slug" params={{ slug: project.slug }} className="block focus-visible:outline focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-amber"><ProjectVisual project={project}/><div className="mt-5 flex items-start justify-between gap-5 border-b border-line pb-4"><div className="min-w-0"><p className="font-mono text-[9px] uppercase text-amber">Project {project.number}</p><h3 className="mt-2 font-serif text-2xl leading-tight sm:text-[1.7rem]">{project.name}</h3></div><span className="mt-1 grid size-9 shrink-0 place-items-center border border-line transition-colors group-hover:border-amber group-hover:bg-amber group-hover:text-primary-foreground"><ArrowUpRight className="size-4 transition-transform group-hover:translate-x-0.5 group-hover:-translate-y-0.5"/></span></div><div className="mt-4 grid gap-4 sm:grid-cols-[1fr_auto]"><p className="max-w-[58ch] text-sm leading-6 text-ink-soft">{project.description}</p><dl className="min-w-28 font-mono text-[9px] uppercase"><dt className="text-ink-soft">Scope</dt><dd className="mt-1 max-w-40">{project.category}</dd></dl></div><p className="mt-4 font-mono text-[9px] uppercase text-ink-soft">Role — <span className="text-ink">{project.role}</span></p></Link></article>)}</div>;
}
...
export function ContactBlock({ includeForm = true, index }: { includeForm?: boolean; index?: string }) {
  const [sent, setSent] = useState(false);
  function submit(event: FormEvent) { event.preventDefault(); setSent(true); }
  return <section className="bg-ink text-paper"><div className={`${pageWrap} grid gap-12 py-20 lg:grid-cols-12 lg:py-28`}><div className={includeForm ? "lg:col-span-7" : "lg:col-span-9"}><p className="eyebrow text-amber">{index ? `${index} / Contact` : "Contact"}</p><h2 className="mt-5 max-w-[18ch] font-serif text-4xl leading-tight sm:text-5xl">Have a product that needs a better interface?</h2><p className="mt-5 max-w-[52ch] text-sm leading-6 text-paper/70">Whether you&apos;re designing a new product, improving an existing interface, or building a scalable design system, I&apos;d be happy to discuss the project.</p><dl className="mt-9 grid gap-5 border-t border-paper/15 pt-6 font-mono text-[10px] uppercase sm:grid-cols-3"><div><dt className="text-paper/45">Email</dt><dd className="mt-1">[ADD EMAIL]</dd></div><div><dt className="text-paper/45">LinkedIn</dt><dd className="mt-1">[ADD LINKEDIN]</dd></div><div><dt className="text-paper/45">Location</dt><dd className="mt-1">Karachi, Pakistan</dd></div></dl></div>{includeForm && <form onSubmit={submit} className="rounded-lg border border-paper/15 p-6 lg:col-span-5"><label className="form-label">Name</label><Input required className="form-input" placeholder="Your name"/><label className="form-label mt-5">Email</label><Input required type="email" className="form-input" placeholder="you@company.com"/><label className="form-label mt-5">Project / Message</label><Textarea required className="form-input min-h-28 resize-none" placeholder="Tell me about the product..."/><Button type="submit" className="mt-5 w-full">Send Message</Button>{sent && <p role="status" className="mt-4 flex gap-2 text-xs text-paper/70"><Check className="size-4 text-amber"/>Thanks — the form is ready to connect once Ahmed&apos;s email is added.</p>}</form>}</div></section>;
}

export function Availability() { return <span className="inline-flex items-center gap-2 rounded-full border border-line bg-paper-2 px-3 py-1.5 font-mono text-[10px] uppercase tracking-[0.14em] text-ink-soft"><Circle className="size-2 fill-current text-amber"/>Available for selected work</span> }